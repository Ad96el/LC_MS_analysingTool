   ...
   # check baseline type
    ...
    for peak in candidates:
        if peak[0] > 0 and (peak[1] - peak[2]) >= threshold and (not peak[3] or peak[3] >= snThreshold):
            ...
            fwhm005 = mod_signal.width(signal, peak[0], (peak[2] + ((peak[1] - peak[2]) * 0.005)))
            centroides.append(obj_peak.peak(mz=peak[0], ai=peak[1], base=peak[2], sn=peak[3], fwhm=peak[4],fwhm005=fwhm005))
