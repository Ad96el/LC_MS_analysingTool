...
double signal_median(double inarr[], int len)
{
    ...
    for (;;)
    {
        ...
        middle = (low + high) / 2;
        if (inarr[middle] > inarr[high])
            ELEM_SWAP(inarr[middle], inarr[high]);
        if (inarr[low] > inarr[high])
            ELEM_SWAP(inarr[low], inarr[high]);
        if (inarr[middle] > inarr[low])
            ELEM_SWAP(inarr[middle], inarr[low]);
        ...
        for (;;)
        {
            do
                ll++;
            while (inarr[low] > inarr[ll]);
            do
                hh--;
            while (inarr[hh] > inarr[low]);
            if (hh < ll)
            {
                break;
            }
...
// apply binary search. What is p_signal? 
{
    int lo = 0;
    int hi = p_signal->len;
    int mid;
...
// search for the maxvalue of an array. iterate through it. 
{
    int cell, i, idx;
    double value;
...
m_arrayd* signal_width_index(m_arrayd *p_signal, double x, double height) {
    int idx, ileft, iright;
    double xleft, xright;
    double x1, x2, y1, y2;
    m_arrayd *out;

    if ((out = (m_arrayd *)malloc(sizeof(m_arrayd))) == NULL)
    {
        return NULL;
    }
    ...
    if ((out->data = (double *)malloc(2 * 2 * sizeof(double))) == NULL)
    {
        return NULL;
    }
...
m_arrayd *array_py2md(PyArrayObject *p_inarr)
{
    ...
    if (dim == 2)
        cell = (int)PyArray_DIM(p_inarr, 1);
...
static PyMethodDef module_methods[] = {
    ...
    {"signal_width_index", _wrap_signal_width, METH_VARARGS, "signal_width_index( PyArray, double, double )"},
...
{
    import_array();
    PyMODINIT_FUNC module = PyModule_Create(&modules);
    return module ;
}
