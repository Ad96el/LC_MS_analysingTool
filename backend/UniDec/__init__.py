...
from .unidec_modules import IM_functions as IM_func
from .unidec_modules import MassSpecBuilder as MSBuild
...
class UniDec(UniDecEngine):
    ...
    def open_file(self, file_name, file_directory=None, time_range=None, *args, **kwargs):
        ...
        # Import Data
        self.data.rawdata = ud.load_mz_file(
            self.config.filename, self.config, time_range, imflag=self.config.imflag)
        ...
        if self.data.rawdata.shape[1] == 3:
            ...
            if len(self.data.rawdata[:, 2]) == len(np.ravel(intgrid)):
                intgrid = self.data.rawdata[:, 2].reshape(
                    (len(mzaxis), len(dtaxis)))
            else:
                for x, y, z in self.data.rawdata:
                    intgrid[np.where(mzaxis == x)[0][0],
                            np.where(dtaxis == y)[0][0]] = z
            self.data.rawdata = np.transpose([mzaxis, np.sum(intgrid, axis=1)])
            mzaxis, dtaxis = np.meshgrid(
                mzaxis, dtaxis, sparse=False, indexing='ij')
            self.data.rawdata3 = np.transpose(
                [np.ravel(mzaxis), np.ravel(dtaxis), np.ravel(intgrid)])
        ...
        if os.path.splitext(self.config.filename)[1] == ".zip":
            ...
            if inflag:
                ...
                newfilepath = os.path.join(os.path.dirname(
                    self.config.dirname), newfilename)
            ...
            if os.path.isfile(newfilepath):
                ...
                if self.config.system == "Windows":
                    if self.config.imflag == 0:
                        ...
                        ud.waters_convert2(
                            self.config.dirname, config=self.config, outfile=newfilepath)
                        ...
                        call = [self.config.cdcreaderpath, '-r', self.config.dirname, '-m',
                                newfilepath[:-10] +
                                "_msraw.txt", '-i', newfilepath, '--ms_bin', binsize,
                        ...
                        if result == 0 and os.path.isfile(newfilepath):
                            ...
                            print("Failed conversion to txt file. ",
                                  result, newfilepath)
                            return None, None
                else:
                    print(
                        "Sorry. Waters Raw converter only works on windows. Convert to txt file first.")
                    return None, None
            return self.config.filename, self.config.dirname

        else:
            print("Error in conversion or file type:",
                  self.config.filename, self.config.dirname)
        pass
    ...
    def process_data(self, **kwargs):
        ...
        if self.config.imflag == 0:
            ...
            self.data.data3 = np.transpose(
                [np.ravel(mz), np.ravel(dt), np.ravel(i3)])
    ...
    def run_unidec(self, silent=False, efficiency=False):
        ...
        if out == 0:
            self.unidec_imports(efficiency, everything=True)
    ...
    def unidec_imports(self, efficiency=False, everything=False):
        ...
        try:
            ...
            self.config.massdatnormtop = np.amax(self.data.massdat[:, 1])
        if not efficiency:
            self.data.massgrid = np.fromfile(
                self.config.massgridfile, dtype=self.config.dtype)
            self.data.fitdat = np.fromfile(
                self.config.fitdatfile, dtype=self.config.dtype)
            try:
        ...
        if self.config.imflag == 0:
            ...
            self.config.error = 1 - sse / \
                np.sum((self.data.data2[:, 1] - mean) ** 2)
            if not efficiency:
                # Import Grid
                try:
                    self.data.mzgrid = np.fromfile(
                        self.config.mzgridfile, dtype=self.config.dtype)
            ...
            if not efficiency:
                ...
                self.data.massccs = self.data.massccs.reshape(
                    (masslen, ccslen))
                self.data.ccsz = self.data.ccsz.reshape((zlen, ccslen))
                self.data.mztgrid = np.clip(
                    self.data.mztgrid, 0.0, np.amax(self.data.mztgrid))
                ...
                xv, yv = np.meshgrid(
                    self.data.ztab, np.unique(self.data.data3[:, 0]))
                xv = np.c_[np.ravel(yv), np.ravel(xv)]
    ...
    def setup_peaks(self, peaks):
        ...
        try:
            mztab = ud.make_peaks_mztab(
                self.data.mzgrid, self.pks, self.config.adductmass)
        except:
            pass
        ...
        try:
            ud.peaks_error_mean(self.pks, self.data.massgrid,
                                self.data.ztab, self.data.massdat, self.config)
        except Exception as e:
            print("Error in error calculations:", e)
        if self.config.batchflag == 0:
            try:
                ud.make_peaks_mztab_spectrum(
                    self.data.mzgrid, self.pks, self.data.data2, mztab)
                self.export_config()
            except:
                pass
    ...
    def kendrick_peaks(self, kmass=None, centermode=1):
        ...
        if self.config.kendrickmass > 0:
            self.pks.get_mass_defects(
                self.config.kendrickmass, mode=centermode)
    ...
    def kendrick_continuous(self, ref_mass=None, centermode=0, nbins=50, transformmode=0, xaxistype=1):
        ...
        if self.config.kendrickmass > 0:
            ...
            outfile2 = os.path.join(
                self.config.outfname + "_2D_Mass_Defects.txt")
            outfile1 = os.path.join(
                self.config.outfname + "_1D_Mass_Defects.txt")
    ...
    def mass_grid_to_f_grid(self):
        ...
        """
        mgrid, zgrid = np.meshgrid(
            self.data.massdat[:, 0], np.array(self.data.ztab), indexing="ij")
        ogrid = ud.get_z_offset(mgrid, zgrid)
        oaxis = np.arange(np.amin(ogrid), np.amax(ogrid), 0.5)
        mgrid2, ogrid2 = np.meshgrid(
            self.data.massdat[:, 0], oaxis, indexing="ij")
        massgrid = self.data.massgrid.reshape(
            (len(self.data.massdat[:, 0]), len(self.data.ztab)))
    ...
    def export_params(self, e):
        ...
        if self.pks.plen > 0:
            ...
            ud.dataexport(mztab[:, :, 1],
                          self.config.outfname + "_chargedata.dat")
            ...
            if self.config.batchflag == 0:
                try:
                    ...
                    ud.dataexport(
                        chargeareas, self.config.outfname + "_chargedata_areas.dat")
            ...
            for i in range(0, len(peaks)):
                ...
                std = np.sqrt(np.average(
                    (np.array(self.data.ztab) - avg) ** 2, weights=mztab[i, :, 1]))
                p = self.pks.peaks[i]
                if e == "PostFit":
                    peakparams.append(
                        [peaks[i, 0], self.config.mzsig * avg, avg, std, peaks[i, 1] / np.sum(peaks[:, 1]),
                         self.massfit[i, 1], self.massfit[i, 2] /
                         np.sum(self.massfit[:, 2]),
    ...
    def process_mass_data(self):
        ...
        if self.config.smooth > 0:
            self.data.massdat = ud.gsmooth(
                self.data.massdat, self.config.smooth)
        ...
        if subtype == 1 and buff != 0:
            ...
            self.data.massdat[:, 1] = self.data.massdat[:,
                                                        1] - np.amin(self.data.massdat[:, 1])
        # Normalization
        self.data.massdat = ud.normalize(self.data.massdat)
        # Intensity Threshold
        self.data.massdat = ud.intensitythresh(
            self.data.massdat, self.config.intthresh)  # thresh
        ...
        if os.path.isfile(self.config.infname):
            if self.config.imflag == 0:
                ...
                self.data.data2 = np.transpose(
                    [np.unique(self.data.data3[:, 0]), np.sum(i3, axis=1)])
            self.config.procflag = 1
        else:
            self.config.procflag = 0
    ...
    def get_zstack(self, xfwhm=1):
        zarr = np.reshape(self.data.massgrid, (len(
            self.data.massdat), len(self.data.ztab)))
    ...
    def pks_mscore(self, xfwhm=2, pow=2):
        ...
        for p in self.pks.peaks:
            ...
            rats = []
            weights = []
            for i, _ in enumerate(self.data.ztab):
                Y = ints[i]  # mzgrid
                sY = np.sum(Y)
    ...
    def get_mzstack(self, xfwhm=2):
        zarr = np.reshape(self.data.mzgrid[:, 2], (len(
            self.data.data2), len(self.data.ztab)))
        ...
        for i, p in enumerate(self.pks.peaks):
            ...
            for j, z in enumerate(self.data.ztab):
                ...
                stack.append(np.transpose(
                    [top[:, 0], top[:, 1], zarr[boo3, j]]))
                bvals.append(boo3)
    ...
    def pks_fscore(self):
        for p in self.pks.peaks:
            ...
            if p.badFWHM:
                if diff[0] > diff[1]:
                    dcut1 = ud.datachop(
                        self.data.massdat, interval[0] - self.config.massbins, p.mass)
                    ...
                    dcut2 = ud.datachop(
                        self.data.massdat, p.mass, interval[1] + self.config.massbins)
    ...
    def dscore(self, xfwhm=2, pow=2):
        ...
        if len(self.pks.peaks) > 0:
            ...
            for p in self.pks.peaks:
                scores = np.array(
                    [p.mscore, p.uscore, p.fscore, p.cs_score])  # p.rsquared,
            ...
            ints = np.array(ints)
            self.pks.uniscore = ud.weighted_avg(
                dscores, ints ** pow) * self.config.error
...
def data_reduction(data : np.ndarray, reduction):

    if data.ndim > 1: 
        data[0,1] = reduction 
        data[len(data) -1 , 1] = reduction  
        to_keep =  data[:,1] >= reduction

    else:
        data[0] = reduction
        data[len(data) - 1] = reduction
        to_keep = data[:,1]  >= reduction

    #for a smooth zoom 
    to_keep[::5000] = True 
    trimed_data = data[to_keep] 
    smoothing = trimed_data[:,1] < reduction
    trimed_data[smoothing , 1] = reduction
    return trimed_data


def get_rawData(eng: UniDec):
    data = np.asanyarray(eng.data.rawdata)
    maxVal = np.amax(data[:,1])
    normalized_values = (data[:,1]) / maxVal
    data[:,1] = normalized_values
    return data



def get_data(eng: UniDec, reduction_decon, reduction_raw) -> Dict[str, Dict]:

    fitted_data = eng.data.fitdat 
    raw_data = data_reduction(get_rawData(eng), reduction_raw)
    processed_data = eng.data.data2
    deconvolve = data_reduction(eng.data.massdat , reduction_decon)


    peaks = [] 
    for index, p in enumerate(eng.pks.peaks):
        out = {}
        out["peakMass"] = round(p.mass, 2)
        out["peakIntensity"] = round(p.height, 2) 
        out["score"] = abs(round(p.uscore * 100, 2)  ) 
        mztab = np.array(p.mztab)
        mztab2 = np.array(p.mztab2)
        maxval = np.amax(mztab[:, 1])
        b1 = mztab[:, 1] > 0.1 * maxval
        y = np.reshape(mztab2[b1, 1], (-1, 1))
        x = np.reshape(mztab2[b1, 0], (-1, 1))
        spectrum = np.concatenate((x, y), axis=1)
        df_spectrum = pd.DataFrame(data=spectrum, columns=["x", "y"])
        df_spectrum["y"] = df_spectrum["y"] * 100  
        out["specToPeak"] = json.loads(df_spectrum.to_json(orient="records"))
        out["id"] = index
        peaks.append(out)

    fitted_data_reshaped = np.reshape(fitted_data, (1, fitted_data.shape[0])) 
    np_fit = np.concatenate((processed_data[:,0].reshape(1,-1).T , fitted_data_reshaped.T), axis=1)
    np_fit = data_reduction(np_fit, reduction_raw)
 
    df_fit = pd.DataFrame(data=np_fit, columns=["x", "y"])
    df_raw = pd.DataFrame(data=raw_data, columns=["x", "y"])

    df_fit["y"] = df_fit["y"] * 100 
    df_raw["y"] = df_raw["y"] * 100  
    df_raw = df_raw.round(2) 
    df_fit = df_fit.round(2)
    df_deconv = pd.DataFrame(data=deconvolve, columns=[
                             "x", "y"]).round({"y": 1, "x": 2}) 
    json_fit = json.loads(df_fit.to_json(orient="records"))
    json_decon = json.loads(df_deconv.to_json(orient="records"))
    json_raw = json.loads(df_raw.to_json(orient="records")) 
    return {"fit": json_fit, "decon": json_decon, "peaks": peaks, "raw" : json_raw }


def run_unidec(file: str, folder: str, **kwargs):
    file_root = file.split(".")[0]
    eng = UniDec()
    configs = UniDecConfig()
    eng.open_file(file, folder)
    configs.setup_parameters(file_root + "_conf.dat", kwargs["params"])
    eng.load_config(file_root + "_conf.dat")
    eng.process_data()
